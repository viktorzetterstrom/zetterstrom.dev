...
docker build --platform linux/amd64 -t viktorzetterstrom/index-zetterstrom-dev:$TAG -f packages/index/Dockerfile packages/index
docker push viktorzetterstrom/index-zetterstrom-dev:$TAG

# Build and push movies service
echo "Building movies service..."
docker build --platform linux/amd64 -t viktorzetterstrom/movies-zetterstrom-dev:$TAG -f packages/movies/Dockerfile .
