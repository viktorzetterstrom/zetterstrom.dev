...
const generateTvSeriesRating = ({ title, rating, year }: Rating) => (
  <li key={title}>
    <span className={`rating rating-${rating}`}>{rating}</span>
    {title} - {year}
  </li>
...
export const Ratings: React.FC<RatingProps> = ({ ratings }) => (
  <div>
    ...
    <div>
      <h2>movies</h2>
      <ul>{ratings.filter(({ type }) => type === FormatType.MOVIE).map(generateMovieRating)}</ul>
    </div>
    <p>--------------------------------------------------------------------</p>
    <div>
      <h2>tv-series</h2>
      <ul>
        {ratings.filter(({ type }) => type === FormatType.SERIES).map(generateTvSeriesRating)}
      </ul>
    </div>
  </div>
)
